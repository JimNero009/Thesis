...
&= -\frac{C_1}{\hat{t}_2} s_{3b} \matel{2}{\mu}{3} + \frac{C_2}{\tilde{t}_2} s_{2b} \matel{2}{\mu}{3} + i \matel{2}{\mu}{3}\frac{C_t}{s_{23}}(\hat{t}_1 + (s_{2b} + s_{3b}) - 2p_b^\mu p_b^\rho + 2 p_b^\rho p_b^\mu) \\
...
and therefore the vertex (and thus the amplitude) is, indeed, a gauge invariant quantity. We see, however, that the effective vertex is not truly factorised since there is a clear dependence on $p_a$. We therefore choose to do our calculations in a gauge where the gluon polarisation vector is orthogonal to $p_a$ and we have;
...
\end{enumerate} 
It is absolutely crucial that all of these points is properly addressed and checked and so such a task is necessarily time consuming. After validating these steps by comparisons to old code with certain restraints, we can be confident that everything works as it should. \todo{Include some comparison here?}
\section{Results}
...
\begin{figure}[H] 
\centering
\subfloat{\includegraphics[scale=0.4]{Images/m12_qqx0.pdf}} 
\subfloat{\includegraphics[scale=0.4]{Images/m12_qqx1.pdf}}
...
Finally, we revisit a four-jet ATLAS analysis \cite{Aad2015}. \todo{Add figure, finish off - 4 jet analysis}
